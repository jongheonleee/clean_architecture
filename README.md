...
> ### 👉 멀티 코어, 멀티 쓰레드, 빅 데이터 -> 함수형 프로그래밍(FP)의 핵심은 "불변과 순수함수"
- FP는 멀티 쓰레드 + 빅 데이터 환경에서 작업
  - 변수가 불변으로 처리되는 것이 중요
  - 가변 변수 + 멀티 쓰레드
    - 경합 조건, 교착 상태 조건, 동시 업데이트 문제 ... 발생
    - 이를 해결하기 위해 FP에서는 불변과 순수함수가 핵심 


> ### 👉 가변 컴포넌트와 불변 컴포넌트로 분리

<img src="https://github.com/jongheonleee/clean_architecture/assets/87258372/00173b25-4ab8-44ff-a3d4-7430664f4ceb" width="500" height="500"/>

- 불변 컴포넌트 = 순수함수 + 불변 변수
  - 불변 컴포넌트와 가변 컴포넌트를 분리해서 관리
<br>

> ## 🧑🏻‍🏫 패러다임 개요 정리
> - 구조적 프로그래밍 : 제어흐름의 직접적인 전환에 대한 규율
> - 객체 지향 프로그래밍 : 제어흐름의 간접적인 전환에 부과되는 규율
> - 함수형 프로그래밍 : 변수 할당에 부과되는 규율 
> - 컴퓨터 프로그램은 순차, 분기, 반복, 참조로 구성되어 있음, 여기에 규율을 추가하면서 프로그래밍함
